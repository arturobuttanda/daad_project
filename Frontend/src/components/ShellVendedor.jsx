...
const elementosNavegacion = [
  { a: "/vendedor", etiqueta: "Inventario", corto: "IV", icono: IconoInventario },
  { a: "/vendedor/reporte", etiqueta: "Finanzas", corto: "FI", icono: IconoReporte },
];

function IconoInventario({ className = "h-5 w-5" }) {
  return (
...
function IconoReporte({ className = "h-5 w-5" }) {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className={className}>
      <path d="M4 19V5" />
      <path d="M4 19h16" />
...
function IconoMas({ className = "h-5 w-5" }) {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className={className}>
      <path d="M12 5v14" />
      <path d="M5 12h14" />
...
function IconoChevronAbajo({ className = "h-4 w-4" }) {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className={className}>
      <path d="m6 9 6 6 6-6" />
    </svg>
...
export default function ShellVendedor({ title, subtitle, children }) {
  ...
  const [formularioPerfil, setFormularioPerfil] = useState(() => ({
    ...
    contrasena: "",
    confirmarContrasena: "",
    alertasActividad: localStorage.getItem("activityAlertsEnabled") !== "false",
  }));
  const ubicacion = useLocation();
  const nombreUsuario = formularioPerfil.nombreUsuario;

  useEffect(() => {
    localStorage.setItem("vendorSidebarCollapsed", String(estaColapsado));
  }, [estaColapsado]);
  ...
  const manejarEnvioPerfil = async (evento) => {
    ...
    localStorage.setItem("activityAlertsEnabled", String(formularioPerfil.alertasActividad));
  ...
  const abrirSeccionPerfil = (seccion) => {
    setSeccionPerfil(seccion);
  };

  const cerrarPanelPerfil = () => {
    setMenuActivo(null);
    setSeccionPerfil("menu");
  };
    ...
    <div ref={refContenedor} className="min-h-screen bg-[#F9F9FF]">
                ...
                <div className="absolute right-0 top-full z-40 mt-3 w-[min(92vw,28rem)] rounded-3xl border border-[#D6DEEE] bg-white p-5 shadow-[0_20px_60px_rgba(11,27,43,0.18)]">
                  ...
                  {seccionPerfil === "menu" ? (
                    <div className="mt-4 space-y-3">
                      ...
                      <button
                        type="button"
                        onClick={() => abrirSeccionPerfil("user")}
                      ...
                      <button
                        type="button"
                        onClick={() => abrirSeccionPerfil("password")}
                        className="flex w-full items-center justify-between rounded-2xl border border-[#E6ECF6] px-4 py-3 text-left transition hover:border-[#9FB3E8] hover:bg-[#EFF4FF] hover:text-ocean"
                      >
                        <span>
                      ...
                      <div className="flex items-center justify-between rounded-2xl border border-[#E6ECF6] bg-[#F8FAFF] px-4 py-3">
                        ...
                        <button
                          type="button"
                          onClick={() => setFormularioPerfil((actual) => ({ ...actual, alertasActividad: !actual.alertasActividad }))}
                          className={[
                            "relative inline-flex h-7 w-12 items-center rounded-full transition",
                            formularioPerfil.alertasActividad ? "bg-ocean" : "bg-slate-300",
                          ].join(" ")}
                          aria-pressed={formularioPerfil.alertasActividad}
                        >
                          <span
                            className={[
                              "inline-block h-5 w-5 transform rounded-full bg-white shadow transition",
                              formularioPerfil.alertasActividad ? "translate-x-6" : "translate-x-1",
                            ].join(" ")}
                          />
                        </button>
                      </div>
                    ...
                    <form className="mt-4 space-y-4" onSubmit={manejarEnvioPerfil}>
                      <label className="block">
                        ...
                        <input
                          ...
                          value={formularioPerfil.nombreUsuario}
                          onChange={(evento) => setFormularioPerfil((actual) => ({ ...actual, nombreUsuario: evento.target.value }))}
                          placeholder="Nombre de cuenta"
                        />
                      </label>

                      <div className="flex gap-3">
                        <button type="button" onClick={() => setSeccionPerfil("menu")} className="secondary-button w-full justify-center">
                          Volver
                        </button>
                    ...
                    <form className="mt-4 space-y-4" onSubmit={manejarEnvioPerfil}>
                      <label className="block">
                        ...
                        <input
                          className="input-field mt-2"
                          type="password"
                          value={formularioPerfil.contrasena}
                          onChange={(evento) => setFormularioPerfil((actual) => ({ ...actual, contrasena: evento.target.value }))}
                      ...
                      <label className="block">
                        ...
                        <input
                          className="input-field mt-2"
                          type="password"
                          value={formularioPerfil.confirmarContrasena}
                          onChange={(evento) => setFormularioPerfil((actual) => ({ ...actual, confirmarContrasena: evento.target.value }))}
      ...
      <div
        className={[
          ...
          estaColapsado ? "lg:grid-cols-[88px_minmax(0,1fr)]" : "lg:grid-cols-[220px_minmax(0,1fr)]",
        ].join(" ")}
      >
        <aside className="group relative border-r border-[#D6DEEE] bg-[#EEF2FF] px-3 py-5 transition-all duration-300">
          <button
            type="button"
            onClick={() => setEstaColapsado((actual) => !actual)}
            className={[
              ...
              estaColapsado ? "opacity-0 group-hover:opacity-100 focus:opacity-100" : "opacity-100",
            ].join(" ")}
            aria-label={estaColapsado ? "Expandir barra lateral" : "Colapsar barra lateral"}
          >
            {estaColapsado ? <IconoChevronDerecha className="h-4 w-4" /> : <IconoChevronIzquierda className="h-4 w-4" />}
          </button>

          <div className={estaColapsado ? "flex flex-col items-center px-1" : "px-2"}>
            <div
              className={[
                "flex items-center gap-3 transition-all duration-300",
                estaColapsado ? "justify-center" : "justify-start",
              ].join(" ")}
            >
              <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-ocean text-white shadow-glow">
              ...
              {!estaColapsado ? (
                <div>
                  <h1 className="font-display text-2xl font-semibold text-ocean">Hola, {nombreUsuario || "Vendedor"}</h1>
            ...
            {elementosNavegacion.map((elemento) => (
              <Link
                ...
                className={[
                  ...
                  estaColapsado ? "justify-center px-2" : "gap-3 px-3",
                  ubicacion.pathname === elemento.a ? "bg-[#6EE7B7] text-[#0F172A]" : "text-slate-700 hover:bg-white",
                ].join(" ")}
                title={estaColapsado ? elemento.etiqueta : undefined}
              >
                ...
                {estaColapsado ? null : <span>{elemento.etiqueta}</span>}
              </Link>
            ))}
          </nav>
          ...
          <Link
            ...
            className={[
              ...
              estaColapsado ? "justify-center px-3" : "",
            ].join(" ")}
            title={estaColapsado ? "Agregar producto" : undefined}
          >
            <span className="inline-flex h-5 w-5 items-center justify-center">
              <IconoMas className="h-5 w-5" />
            </span>
          {estaColapsado ? null : <span className="ml-2">Agregar producto</span>}
          </Link>
          ...
          >
            ...
            {estaColapsado ? null : <span className="ml-2">Cerrar sesión</span>}
          </button>
        </aside>
        ...
        <main className="p-4 sm:p-6 lg:p-8">
          <header className="mb-6 flex flex-wrap items-center justify-between gap-4 border-b border-[#D6DEEE] pb-4">
            ...
            <div className="text-right text-sm text-slate-600">
              <p>{nombreUsuario || "Vendedor"}</p>
              <p className="text-xs">Nexus Socio</p>
            </div>
          </header>
